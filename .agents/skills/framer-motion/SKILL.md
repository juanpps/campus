...
| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `mindrally-skills-framer-motion`                                |
| Version    | 1.0.1                                     |
| Author     | Mindrally                                 |
...
**GitHub:** [Mindrally/skills](https://github.com/Mindrally/skills) — ⭐ 22 | Forks: 2
...
This Skill provides expert guidelines for building performant, maintainable animations with the Motion (formerly Framer Motion) library in React + TypeScript projects. It explains the correct import (import { motion } from "motion/react"), a performance-first approach that animates transform and opacity (hardware-accelerated) while avoiding layout-shifting properties, and proper use of will-change (transform, opacity, filter, clipPath, backgroundColor). It covers best practices such as using variants and staggered transitions for complex sequences, layoutId for shared element transitions, and preferring spring-based transitions for natural motion. React integration guidance includes memoization patterns and TypeScript-typed components to minimize re-renders. Use cases include micro-interactions, list staggering, page and shared-element transitions, and high-performance UI animations that remain smooth across devices.
...
```bash
npx -y @lobehub/market-cli skills install mindrally-skills-framer-motion
```
...
npx -y @lobehub/market-cli skills install mindrally-skills-framer-motion --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install mindrally-skills-framer-motion --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install mindrally-skills-framer-motion --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install mindrally-skills-framer-motion --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install mindrally-skills-framer-motion --version 1.0.1
```
...
```bash
npx -y @lobehub/market-cli skills rate mindrally-skills-framer-motion --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment mindrally-skills-framer-motion -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment mindrally-skills-framer-motion
```
...
```bash
npx -y @lobehub/market-cli skills comments mindrally-skills-framer-motion
```
